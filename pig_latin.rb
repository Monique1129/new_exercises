...
def pig_latin(word)
  if word.chr == "a" || word.chr == "e" || word.chr == "i" || word.chr == "o" || word.chr == "u" #first character checks if vowel or consonant, if true appends "ay"
    ...
    arr = word.split(//) #converts string into array, each character as element in the array
    ind = word.index(/[aeiou]/) #returns index of character with first vowel
    if ind == nil
      word << "ay"
    else
      latin = arr.drop(ind) << arr.slice(0..ind-1) << "ay"
      latin.flatten.join('') #convert array back into string
    end
  end
end
...
puts "Enter Word to be Converted into Pig Latin"
